    ...
    <modal-sm :details="modalDetails" @close-modal="modalDetails.show = false">
        ...
        <div class="row px-4">
            <div class="col-sm-12 col-md-12 col-lg-12 mb-4">
                <div>
                    <button class="btn btn-primary w-100" @click="proceed()">Select</button>
                </div>
            </div>
        </div>
    </modal-sm>
...
export default defineComponent({
    ...
    setup() {
        ...
        const proceed = () => {
            setParams(patientType.value)
            console.log(params.value)
            const paramsString = encryptData(JSON.stringify(params.value));
            router.push({ name: 'chart', params: { data: encodeURIComponent(paramsString) } })
        }
